# КОНТЕКСТ ПРОЕКТА: SaldaCargo (14.05.2026 — сессия 8)

> Этот файл — точка входа для AI-ассистента. Прочитав его, ты знаешь где мы находимся и что делать дальше.
> База знаний: `kb/wiki/` — читай соответствующие статьи перед задачами.

---

## 1. Что за проект

SaldaCargo — ERP-система для транспортной компании (Верхняя Салда).  
Цель: в будущем продать как коробочное решение для других транспортных компаний.

**Два приложения:**

- `apps/miniapp` — Next.js 15, мобильный интерфейс для водителей/механиков/админа (мессенджер MAX)
- `apps/web` — Next.js 15, десктопный дашборд для владельца и админа

**Стек:** Next.js 15, React 19, TypeScript, Supabase (PostgreSQL), TanStack Query, Tailwind CSS 4  
**Auth:** кастомная cookie-схема (`salda_user_id`), НЕ Supabase Auth  
**Деплой:** Vercel (miniapp и webapp — оба задеплоены, автодеплой из `main`) — **всегда пушить после коммита**

---

## 2. Что уже реализовано

### MiniApp (apps/miniapp) — ГОТОВО ✅

- **Авторизация:** выбор роли → выбор имени → PIN-код для администраторов
- **Водитель:** создание рейса, заказы, расходы, завершение с одометром, заявка на ремонт
- **Механик:** наряды, запчасти, статусы. Видит только `lifecycle_status='approved'`
- **Админ (мобильный):**
  - Дашборд KPI
  - Рейсы: На ревью / Активные / История (навигация по датам)
  - Наряды: На ревью / Активные / История
  - Финансы: Доход / Расход / Долги (кредиты + поставщики) / Инкассация / ЗП
  - **Дебиторка:** список должников → заказы → погасить
  - **Журнал транзакций:** навигация по дням (← →) и месяцам, итоги за день
  - **Долги поставщиков** (`/api/admin/payables`): считает из транзакций, показывает Дерябин ГСМ / Новиков / Ромашин

### WebApp (apps/web) — В РАЗРАБОТКЕ 🔨

Все разделы существуют и работают:

- **/ (главная):** KPI за месяц
- **/review:** одобрение/возврат рейсов + история + **итоги за день и за месяц** ✅
- **/retro:** ручной ввод рейса задним числом
- **/finance:** P&L за 6 месяцев + журнал с навигацией по дням/месяцам + подотчёт водителей
- **/receivables:** дебиторка — список должников, кнопка "Оплачено", добавление исторических долгов ✅
- **/counterparties:** контрагенты — Постоянные / Новые, объединение дублей, защита от дублирования ✅
- **/fleet:** автопарк — коллапсируемые плитки, юнит-экономика ✅
- **/garage:** наряды СТО ✅
- **/staff:** персонал — выплаты ЗП, CRUD ✅
- **/loans:** кредиты и займы ✅
- **/payables:** долги поставщикам — Дерябин ГСМ / Новиков / Ромашин ✅

---

## 3. Что сделано в сессии 8

### PIN-защита для администраторов (MiniApp)

`apps/miniapp/app/page.tsx` — при выборе роли "Администрация" и нажатии на имя:

- Если `has_pin=true` (возвращается из `/api/users/public`) — показывается экран с цифровой клавиатурой (4 точки прогресса, кнопки 0–9, C, ⌫)
- PIN проверяется серверно через `POST /api/auth/verify-pin`
- Cookie устанавливается только после успешной проверки

**Пины:**

- Нигамедьянов А.С. → `3030`
- Шахмаев А.О → `9111`

**Безопасность:** `pin_code` не возвращается в публичном API — только флаг `has_pin: boolean`. Верификация только на сервере.

**Миграция:** `20260514000004_users_pin_code.sql` — добавлена колонка `pin_code TEXT` в `users`, применена через `supabase db push`.

**История миграций:** после применения были помечены как applied через `supabase migration repair` все 20 предыдущих миграций (remote история была пустой).

### Итоги за месяц в разделе История (WebApp /review)

`apps/web/app/(dashboard)/review/page.tsx` + `apps/web/app/api/trips/route.ts`:

- API: добавлен параметр `?month=YYYY-MM` — выборка всех approved рейсов за месяц (UTC-границы)
- UI: второй `useQuery` загружает все рейсы месяца; новая полоса "Итого за месяц · N рейсов · [название месяца]" (тёмно-синяя, `bg-sky-950`) показывается после итогов за день в режиме История

### Дерябин ГСМ (переименование Опти24)

`apps/web/app/api/payables/route.ts` + UI:

- Опти24 переименован в **Дерябин ГСМ** везде (API, payables, finance)
- Логика `autoAccrue=true` (fuel_card расходы → автодолг) **сохранена**
- Добавлена кнопка "+ Добавить долг" для Дерябина (раньше была только у Новикова/Ромашина)
- Расчёт долга Дерябина: `fuel_card расходы + ручные pending транзакции − платежи`
- Информационная строка "⚡ Расходы ГСМ по карте автоматически попадают в долг" под кнопками

### Контрагенты — улучшения (WebApp /counterparties)

- Разделение на "Постоянные клиенты" (is_regular=true) и "Новые клиенты" (is_regular=false)
- Кнопка "★ В постоянные" / "↓ В новые" в раскрытой карточке
- Режим объединения дублей с preview: показывает сумму заказов и выручки до подтверждения
- Защита от дублирования: кнопка "Создать нового клиента" появляется только если совпадений 0
- Телефоны отформатированы через `formatPhone()` из `@saldacargo/shared`

---

## 4. Важные баги исправленные в сессиях 6–7

### Автопарк — выручка считалась неправильно (сессия 6)

`/api/fleet/route.ts` — убран фильтр `settlement_status='completed'` из запроса выручки.  
Выручка теперь = `lifecycle_status='approved'` (работа сделана), независимо от оплаты.

### Дебиторка — кнопка "Оплачено" не работала в WebApp (сессия 6)

`/api/receivables/[orderId]/route.ts` — WebApp работает без cookie (`salda_user_id`), из-за чего `created_by=null` падало с NOT NULL constraint.  
**Фикс:** если нет cookie → берём первого admin из БД. Fallback на hardcoded UUID `e9a1c980-eb1e-5c87-9f6d-c7f67eb28a1d` (Шахмаев А.О).

### MiniApp поставщики запчастей не отображались (сессия 6)

`/api/admin/payables/route.ts` — читал `counterparties.payable_amount` (всегда 0 у Новикова/Ромашина).  
**Фикс:** считает долг из транзакций (pending expense - completed expense).

### Журнал финансов показывал задвоение расходов (сессия 7)

`/api/finance/route.ts` — журнал не фильтровал по `settlement_status`, показывал и pending (долг) и completed (оплата).  
**Фикс:** добавлены `.eq('lifecycle_status', 'approved').eq('settlement_status', 'completed')` в запрос журнала.  
Теперь в журнале — только реально прошедшие деньги.

---

## 5. Дебиторка — исторические долги (сессия 7) ✅

Реализован механизм добавления долгов, возникших ДО старта системы.

**Таблица:** `manual_receivables` (counterparty_id, amount, date, description, settled, settled_at)  
**API:**

- `GET /api/receivables` — объединяет `trip_orders` и `manual_receivables` в один список
- `POST /api/receivables` — создать ручную запись долга
- `PATCH /api/receivables/manual/[id]` — погасить (→ settled=true + income-транзакция)
- `DELETE /api/receivables/manual/[id]` — удалить ошибочно введённую запись

**UI:** кнопка "Добавить долг" в шапке `/receivables`. Ручные записи помечены меткой "Ист. долг" (синяя). Иконка корзины — удалить запись до погашения.

---

## 6. Архитектура учёта ЗП (важно!)

| Источник                              | Что даёт                 | Где используется            |
| ------------------------------------- | ------------------------ | --------------------------- |
| `trip_orders.driver_pay / loader_pay` | Начисленная ЗП из рейсов | P&L, "заработано" на /staff |
| `transactions` (category PAYROLL\_\*) | Факт выплаты денег       | "выплачено" на /staff, debt |

**PAYROLL-транзакции исключены из P&L** чтобы не задваивать.  
**Расчёт долга:** `debt = MAX(earned - paid, 0)`. `auto_settle=true` → `debt=0` всегда.

---

## 7. Дебиторка — архитектура (важно!)

- **Источник долга из рейсов:** `trip_orders` где `settlement_status='pending'` AND `lifecycle_status='approved'`
- **Исторические долги:** `manual_receivables` где `settled=false`
- **"физ лицо" с описанием** → выносится в отдельную строку должника (по `description` поля заказа)
- **Погашение trip_order:** PATCH `/api/receivables/[orderId]` → `settlement_status='completed'` + INSERT income-транзакция
- **Погашение manual:** PATCH `/api/receivables/manual/[id]` → `settled=true` + INSERT income-транзакция

---

## 8. Долги поставщиков — архитектура

**Оба приложения** считают долг одинаково из `transactions`:

- Долг = SUM(expense, pending, approved) - SUM(expense, completed, approved) по `counterparty_id`
- Дерябин ГСМ — `fuel_card` расходы из `trip_expenses` + ручные pending транзакции − платежи

**Чтобы добавить долг** (любой поставщик):

1. WebApp → /payables → "+ Добавить долг" → создаёт `expense` транзакцию `settlement='pending'`
2. После этого появляется в MiniApp в разделе "Долги поставщиков"

**Важно:** pending expense-транзакции (долги поставщиков) НЕ показываются в журнале финансов — только completed.

---

## 9. P&L vs Балансы — что что считает

| Раздел                | Источник выручки                                                          | Источник расходов                                                                   |
| --------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **P&L (/finance)**    | `trip_orders.amount` (approved)                                           | `transactions` (expense, approved, completed) + ЗП из trip_orders                   |
| **Автопарк (/fleet)** | `trip_orders.amount` (approved)                                           | `trip_expenses` + `driver_pay/loader_pay` + `part_movements` + `monthly_fixed_cost` |
| **Балансы кошельков** | `transactions` (income, approved, completed, to_wallet_id)                | `transactions` (expense, approved, completed, from_wallet_id)                       |
| **Дебиторка**         | `trip_orders` (settlement=pending) + `manual_receivables` (settled=false) | —                                                                                   |
| **Журнал финансов**   | `transactions` (approved, completed)                                      | `transactions` (approved, completed)                                                |

> Транзакции "Прочий доход" **не** влияют на P&L. Влияют только на баланс кошельков.  
> Pending expense-транзакции (долги поставщиков) **не** показываются в журнале.

---

## 10. Следующие шаги (в порядке приоритета)

### ШАГ 1 — Починить переключатель периода (WebApp)

Кнопки периода в layout.tsx пишут `?period=` в URL, но страницы его не всегда читают.  
Проверить: fleet, главная, finance, staff, receivables.

### ШАГ 2 — WebApp: активные рейсы могут не отображаться

Рейсы `status='in_progress'` фильтруются по `started_at`, могут пропасть если начались в другой день.

---

## 11. Ключевые технические факты

- `users.roles` — массив: `.contains('roles', ['driver'])`, НЕ `.eq('role', ...)`
- Все API-роуты: `createAdminClient()` + `(supabase.from('table') as any)`
- Деньги — только строкой: `"5000.00"`, компонент `<Money amount="5000.00" />`
- `crypto.randomUUID()` для idempotency_key в webapp (НЕ пакет `uuid`)
- `useSearchParams()` в Next.js 15 — обязательно в `<Suspense>`
- WebApp работает **без cookie** (auth отключена локально) → API не должны требовать cookie для базовых операций
- Скроллируемый flex-child: обязательно `min-h-0` + `overflow-y-auto`
- Bottom-sheet в MAX WebView: `position:fixed` на body + `paddingBottom: calc(env(safe-area-inset-bottom,0px) + 56px)`
- Контрагент при редактировании: find-or-create через `.ilike('name', name).maybeSingle()`
- Двухшаговое удаление: boolean state + useRef таймер 4 сек авто-сброс
- Новые таблицы в Supabase требуют явного `GRANT ALL ON TABLE ... TO postgres, anon, authenticated, service_role`
- История миграций Supabase пуста (remote) — при новых `db push` нужно сначала `migration repair --status applied` для старых

### ID кошельков:

```
10000000-0000-0000-0000-000000000001 — Расчётный счёт
10000000-0000-0000-0000-000000000002 — Сейф (Наличные)
10000000-0000-0000-0000-000000000003 — Карта
```

### ID поставщиков:

```
20000000-0000-0000-0000-000000000001 — Дерябин ГСМ (autoAccrue: fuel_card → долг)
20000000-0000-0000-0000-000000000002 — Новиков А.В. Запчасти
20000000-0000-0000-0000-000000000003 — Ромашин Запчасти
```

### ID категорий транзакций:

```
74008cf7-0527-4e9f-afd2-d232b8f8125a — TRIP_REVENUE
600e7f70-2797-474d-948b-432230036d67 — SERVICE_REVENUE
9d18370d-3228-4f2a-8530-52b168cfa8d7 — REPAIR_PARTS
62cebf3f-9982-4cc6-904b-48c6169cf5e4 — FUEL
d79213ee-3bc6-4433-b58a-ca7ea1040d00 — PAYROLL_DRIVER
18792fa8-fda8-472d-8e04-e19d2c6c053c — PAYROLL_LOADER
3d174f9f-34c2-4bc8-a3a9-d82f96f85bf6 — PAYROLL_MECHANIC
68225ea2-d7de-4442-8ed8-ce2366b5d369 — OTHER_INCOME
df1022df-4ea6-46fc-b9aa-f3c9eb4e7f30 — OTHER_EXPENSE
```

→ Полная схема БД: [`kb/wiki/Справочник_таблиц_БД.md`](../../kb/wiki/Справочник_таблиц_БД.md)  
→ API miniapp: [`kb/wiki/MiniApp_API.md`](../../kb/wiki/MiniApp_API.md)  
→ Структура проекта: [`kb/wiki/Структура_монорепозитория.md`](../../kb/wiki/Структура_монорепозитория.md)

---

_Обновлён: 14.05.2026 (сессия 8). Добавлено: PIN-защита администраторов, итоги за месяц в Ревью, Дерябин ГСМ (кнопка добавить долг), улучшения контрагентов._

---

## Сессия 9 (14.05.2026)

### Редизайн главной страницы WebApp

**Что сделано:** переписан `apps/web/app/(dashboard)/page.tsx` в стиле макета `dashboard-web-a.html` («Оперативный центр»).

**Новая структура страницы (5 секций):**

1. **Счета компании** — 3 градиентные карточки (Банк/Касса/Карта) + полоса подотчёта водителей
2. **Сейчас** — сетка 3×1: рейсы в пути / на ревью / выручка сегодня
3. **Алерты** — карточка «⚡ Требует внимания» с AlertRow (цветовая полоса слева, иконка, описание, сумма, дата, ссылка)
4. **Месяц** — 4 компактные MiniKPICard (Выручка / Расходы / Прибыль / ГСМ+ЗП)
5. **Последние операции** — таблица транзакций (сохранена без изменений)

**Добавлены запросы данных:**

- `/api/trips?status=in_progress&lifecycle=draft` — активные рейсы (названия машин в чипе)
- `/api/alerts` — fleet/loans/receivables алерты (используется для построения строк алертов)
- `/api/receivables/summary` — для строки дебиторки в алертах

**Сохранено без изменений:**

- Все существующие API-роуты
- Layout.tsx (header, навигация)
- Все остальные разделы (/review, /fleet, /finance, ...)

**Технические решения:**

- Градиенты карточек через inline `style={{ background: 'linear-gradient(...)' }}`
- Декоративные круги (blob) через абсолютное позиционирование с `bg-white/10`
- AlertRow — ссылка на раздел через `<a href>`, цветовая кодировка critical/warning/info
- `plural()` и `daysBetween()` — утилиты в том же файле

**Известные проблемы:**

- `/api/receivables/summary` — ошибка 500 (возможно, отсутствует таблица `receivable_follow_ups`). Не влияет на главную — строка дебиторки просто скрывается.

**Сервер:** запущен локально на `http://localhost:3000` (Next.js 16.2.4, Turbopack).

### Правки после ревью (15.05.2026)

**Что исправлено:**

- Удалена секция «Месяц» (KPI-карточки) — её нет в референсе
- Удалена секция «Последние операции» (таблица транзакций) — её нет в референсе
- Удалён компонент MiniKPICard и связанные типы (Summary.month, Summary.recentTransactions)
- Плитка «Рейсы в пути» теперь кликабельна → `/review`
- Плитка «На ревью» теперь кликабельна → `/review`
- В алертах Fleet: заголовок включает госномер `(reg_number)`
- Тип ActiveTrips: добавлен `reg_number` в asset

### Устранение дублирования дебиторки (15.05.2026)

**Проблема:** было две дебиторки — вкладка `recv` внутри `/finance` и отдельная страница `/receivables`. Алерт на главной вёл на `/receivables`, хедер «Финансы» — на `/finance` (с вкладкой дебиторки).

**Решение:** дебиторка осталась только на `/receivables`. Из `/finance` убрано:

- Тип `Tab`: `'expenses' | 'recv' | 'loans' | 'payables'` → `'expenses' | 'loans' | 'payables'`
- `ReceivablesPanel` (585 строк) удалён полностью
- `recvSummary` useQuery удалён
- Вкладка `recv` из таб-бара удалена
- Добавлена кнопка-ссылка «📋 Дебиторка →» (пунктирная рамка, ведёт на `/receivables`)

**Результат:** теперь дебиторка в одном месте — `/receivables`. Из любого входа (алерт, хедер-финансы) пользователь попадает на одну и ту же страницу.

### Компактный редизайн главной (15.05.2026)

**Что изменено:**

- Плитки «Счета компании»: `p-5 min-h-[100px]` → `p-3 min-h-[60px]`, `text-xl` → `text-base`, blob'ы меньше, `rounded-xl` → `rounded-lg`
- Плитки «Сейчас»: `p-5 text-3xl` → `p-3 text-xl`, `rounded-xl` → `rounded-lg`
- Алерты: `px-5 py-3` → `px-3 py-1.5`, заголовок и описание в одну строку через `flex items-center gap-1.5`, `w-1` → `w-0.5`
- Общие отступы: `space-y-5 p-6` → `space-y-3 p-4`, `gap-3` → `gap-2`
- Подотчёт: `px-5 py-3` → `px-3 py-2`, `Подотчёт водителей:` → `Подотчёт:`
- Все `shadow-md` → `shadow-sm`

**Результат:** экран вмещает ~ вдвое больше информации, всё компактно и читаемо.
