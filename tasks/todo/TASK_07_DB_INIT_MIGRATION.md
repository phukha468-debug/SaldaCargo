# TASK 07: Инициализация БД — первая миграция Supabase

**Адресат:** Claude Code.
**Длительность:** 60–90 минут.
**Зависимости:** TASK_06 выполнен. Supabase проект создан (см. шаг 1).

---

## Цель

Создать первую миграцию БД с полной схемой таблиц для SaldaCargo. Применить к Supabase. Настроить RLS (Row Level Security). Сгенерировать TypeScript типы из схемы.

---

## Перед стартом — нужно от пользователя

**Если Supabase ещё не настроен**, попроси пользователя:

1. Создать проект на https://supabase.com (New Project)
2. Назвать: `saldacargo`
3. Регион: `eu-central-1` (Frankfurt, ближайший к Верхней Салде с хорошим пингом)
4. Записать: Project URL и anon key (Settings → API)
5. Создать `.env.local` в корне `C:\salda`:
...
Это создаст файл `supabase/migrations/<timestamp>_init_schema.sql`.

### Шаг 2. Написать SQL схему

Вставь следующий SQL в созданный файл миграции:
...
Или через Supabase Dashboard → SQL Editor → вставить и запустить.

### Шаг 4. Сгенерировать TypeScript типы

```powershell
pnpm gen:types
```

Это перезапишет `packages/shared-types/src/database.types.ts` реальными типами из БД.
...
### Шаг 6. Установить Supabase JS клиент в приложениях

```powershell
pnpm add @supabase/supabase-js --filter @saldacargo/web
pnpm add @supabase/supabase-js --filter @saldacargo/miniapp
pnpm add @supabase/ssr --filter @saldacargo/web
pnpm add @supabase/ssr --filter @saldacargo/miniapp
```

### Шаг 7. Создать Supabase клиент для apps/web
...
Создай аналогичные файлы для `apps/miniapp/lib/supabase/`.

### Шаг 8. Коммит

```powershell
git add .
git commit -m "feat: первая миграция БД и Supabase клиенты

- Полная схема: users, assets, trips, trip_orders, transactions
- ENUMs, индексы, RLS политики
- updated_at триггеры
- Supabase JS клиенты для web и miniapp
...
- [ ] `supabase/migrations/<timestamp>_init_schema.sql` создан
- [ ] Миграция применена к Supabase проекту
- [ ] `packages/shared-types/src/database.types.ts` содержит реальные типы (не placeholder)
- [ ] `apps/web/lib/supabase/client.ts` и `server.ts` созданы
- [ ] `apps/miniapp/lib/supabase/client.ts` и `server.ts` созданы
...
Суpabase проект: https://xxxx.supabase.co
Миграция: supabase/migrations/<timestamp>_init_schema.sql
Таблиц создано: XX
Типы сгенерированы: да
