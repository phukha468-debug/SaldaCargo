...
export default function TripDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center p-6 text-center">
        <div>
          ...
          <button onClick={() => router.push('/')} className="mt-4 text-orange-600 font-black uppercase tracking-widest text-sm">
            На главную
          </button>
        </div>
      </div>
  ...
  const PAYMENT_ICONS: Record<string, string> = {
    cash: 'payments',
    qr: 'qr_code_2',
    bank_invoice: 'account_balance',
    debt_cash: 'sync_alt',
    card_driver: 'credit_card',
    fuel_card: 'local_gas_station',
  };
    ...
    <div className="min-h-screen bg-zinc-50 pb-40">
      ...
      <header className="bg-white border-b-2 border-zinc-200 px-4 h-16 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/')} className="text-zinc-500 text-2xl active:scale-95 transition-transform">
            ←
          </button>
          <span className="text-xl font-black text-zinc-900 uppercase tracking-tight">Рейс №{trip.trip_number}</span>
        </div>
        <LifecycleBadge
      ...
      <main className="p-4 space-y-6 max-w-4xl mx-auto">
        ...
        <section className="bg-white border-2 border-zinc-200 rounded-lg p-4 shadow-sm relative overflow-hidden flex flex-col justify-between items-start gap-1">
          ...
          <h1 className="font-black text-xl text-zinc-900 leading-tight">
            {trip.asset.short_name} <span className="text-zinc-400 font-bold ml-1">{trip.asset.reg_number}</span>
          </h1>
          <p className="font-bold text-sm text-zinc-500 uppercase tracking-wide">
            {trip.driver.name} {trip.loader ? `+ ${trip.loader.name}` : ''} · {isActive ? <TripDuration startedAt={trip.started_at} /> : formatDate(trip.started_at)}
          </p>
        </section>
        ...
        <section className="grid grid-cols-2 gap-3">
          ...
          <StatCard label="Выручка" value={<Money amount={totals.revenue.toString()} />} highlighted />
          ...
          {activeOrders.length === 0 ? (
            ...
            <div className="space-y-3">
              {activeOrders.map((order) => (
                <div
                  key={order.id}
                  className="bg-white border-2 border-zinc-200 rounded-lg p-4 flex justify-between items-center relative overflow-hidden active:bg-zinc-50 transition-colors"
                >
                  ...
                  <div className="flex-1 pl-2">
                    ...
                    <p className="text-[10px] font-bold text-green-600 uppercase tracking-widest">ЗП: <Money amount={order.driver_pay} /></p>
                  </div>
        ...
        <section className="space-y-3">
          ...
          {(trip.trip_expenses ?? []).length === 0 ? (
            <div className="bg-zinc-100 border-2 border-zinc-200 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-center opacity-60">
              <p className="font-bold text-zinc-400 uppercase tracking-tight text-xs">Расходов пока нет</p>
            </div>
          ) : (
            <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1 pl-2">
                    ...
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
                      {expense.description ?? (expense.payment_method === 'fuel_card' ? 'Топливо' : 'Расход')}
                    </p>
                  </div>
                  <div className="text-right">
                    <Money amount={expense.amount} className="font-black text-lg text-red-600" />
                  </div>
                </div>
              ))}
            </div>
...
function StatCard({ label, value, highlighted = false, error = false }: { label: string; value: React.ReactNode, highlighted?: boolean, error?: boolean }) {
  return (
    <div className="bg-white border-2 border-zinc-200 rounded-lg p-3 flex flex-col items-center justify-center text-center shadow-sm">
      <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">{label}</span>
      <span className={`text-lg font-black tracking-tight ${highlighted ? 'text-orange-600' : error ? 'text-red-600' : 'text-zinc-900'}`}>
        {value}
      </span>
    </div>
  );
