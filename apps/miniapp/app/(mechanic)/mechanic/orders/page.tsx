...
export default function MechanicOrdersPage() {
  ...
  const { data: orders, isLoading } = useQuery<ServiceOrder[]>({
    queryKey: ['mechanic-orders', MOCK_MECHANIC_ID, tab],
    queryFn: async () => {
      const status = tab === 'completed' ? 'completed' : '';
      const res = await fetch(`/api/mechanic/orders?mechanic_id=${MOCK_MECHANIC_ID}&status=${status}`);
      ...
      <div className="bg-white border-b border-slate-200 sticky top-16 z-40 px-4 flex gap-6 h-12 items-center">
        <button
          ...
          className={cn(
            ...
            tab === 'active' ? "text-orange-600 border-orange-600" : "text-slate-400 border-transparent"
          )}
        >
          Активные
        </button>
        <button
          ...
          className={cn(
            "h-full text-xs font-black uppercase tracking-widest border-b-4 transition-colors",
            tab === 'completed' ? "text-orange-600 border-orange-600" : "text-slate-400 border-transparent"
          )}
        >
          История
        </button>
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm active:bg-slate-50 transition-colors flex items-center justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className={cn("text-[8px] font-black uppercase px-1.5 py-0.5 rounded", priorityColors[order.priority])}>
              {order.priority === 'urgent' ? '🔥 Срочно' : order.priority === 'normal' ? 'Норм' : 'Низкий'}
            </span>
          </div>
