...
export default function ReviewPage() {
  ...
  const { data: trips = [], isLoading, isError, error } = useQuery<TripForReview[]>({
    queryKey: ['trips-review', selectedDate, showAllPending],
    queryFn: async () => {
      const url = showAllPending
        ? '/api/trips?lifecycle=draft'
        : `/api/trips?lifecycle=draft&date=${selectedDate}`;
    ...
    <div className="p-gutter lg:p-container-padding space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            />
            <span className="text-sm font-medium text-slate-600">Все ожидающие</span>
          </label>
          <div className={`relative transition-opacity ${showAllPending ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
            <input
              type="date"
              value={selectedDate}
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
          <button
            ...
            className={`px-6 py-2 rounded-md text-sm font-semibold transition-all ${
              tab === 'drivers' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            Рейсы водителей
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-md text-sm font-medium transition-all ${
              tab === 'mechanics' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            Наряды механиков
          </button>
      ...
      {isLoading && (
        <div className="grid grid-cols-1 gap-6">
          {[1, 2].map((i) => (
            <div key={i} className="bg-white border border-slate-200 rounded-lg h-64 animate-pulse shadow-sm" />
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider">КЛИЕНТ</th>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider">СУММА</th>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider text-right">ЗП ВОД.</th>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider text-center">% СПЛИТ</th>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider text-right">ЗП ГРУЗ.</th>
                      <th className="px-6 py-2.5 font-label-caps text-[10px] text-slate-500 uppercase tracking-wider">ОПЛАТА</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-body-sm">
                    {activeOrders.map((order) => {
                      ...
                      const splitPercent = amountNum > 0 ? Math.round((driverPayNum / amountNum) * 100) : 0;
                      
                      return (
                        <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-3 font-medium text-slate-900">{order.counterparty?.name ?? '—'}</td>
                          <td className="px-6 py-3 font-data-mono font-semibold"><Money amount={order.amount} /></td>
                          <td className="px-6 py-3 font-data-mono text-right text-emerald-600 font-bold"><Money amount={order.driver_pay} /></td>
                          <td className="px-6 py-3 text-center text-slate-400 font-medium">{splitPercent}%</td>
                          <td className="px-6 py-3 font-data-mono text-right font-medium"><Money amount={order.loader_pay} /></td>
                          <td className="px-6 py-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${PAYMENT_STYLES[order.payment_method] || 'bg-slate-100'}`}>
              ...
              <div className="px-6 py-3 grid grid-cols-2 md:grid-cols-5 gap-4 border-t border-slate-100 bg-slate-50/30">
                <div className="flex flex-col">
                  <span className="font-label-caps text-[9px] text-slate-400 uppercase tracking-wider font-bold">ВЫРУЧКА</span>
                  <span className="font-data-mono text-sm font-semibold text-slate-900"><Money amount={revenue.toString()} /></span>
                </div>
                <div className="flex flex-col">
                  <span className="font-label-caps text-[9px] text-slate-400 uppercase tracking-wider font-bold">ИТОГО ЗП</span>
                  <span className="font-data-mono text-sm font-semibold text-rose-600">-<Money amount={(driverPay + loaderPay).toString()} /></span>
                </div>
                <div className="flex flex-col">
                  <span className="font-label-caps text-[9px] text-slate-400 uppercase tracking-wider font-bold">ТОПЛИВО</span>
                  <span className="font-data-mono text-sm font-semibold text-slate-600">—</span>
                </div>
                <div className="flex flex-col">
                  <span className="font-label-caps text-[9px] text-slate-400 uppercase tracking-wider font-bold">ПРИБЫЛЬ</span>
                  <span className="font-data-mono text-sm text-emerald-700 font-bold"><Money amount={profit.toString()} /></span>
                </div>
                <div className="flex flex-col">
                  <span className="font-label-caps text-[9px] text-slate-400 uppercase tracking-wider font-bold">ПРОБЕГ</span>
                  <span className="font-data-mono text-sm font-semibold text-slate-900">{mileage} км</span>
                </div>
              </div>
        ...
        {!isLoading && trips.length === 0 && (
          <div className="text-center py-20 bg-white border border-slate-200 rounded-lg shadow-sm">
            ...
            <p className="text-slate-500 font-medium">Нет рейсов на ревью за эту дату</p>
          </div>
        )}
      </div>
    </div>
