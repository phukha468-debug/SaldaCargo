...
async function main() {
  const { data: trips } = await supabase.from('trips').select('*').eq('trip_number', 369).single();
  ...
  console.log(
    'All Txs (including cancelled maybe, if I filter it? Oh, my previous query fetched all lifecycle_status):',
  );
  allTxs.forEach((t) => {
